...
宿主订阅会话事件流，再与 **agent 注册表**核对，最后归约成一个状态：
...
- **本宿主启动前就已开始的回合也算数**。流式输出、工具调用、步骤推进只可能发生在回合内，所以它们会自行把回合打开——中途挂载、漏掉一帧都能自愈。
- **上游放弃的回合也会结束**。中断一个回合时，harness **有时根本不追加 `turn/end`**；只跟随日志的监听者会因此永远认为该会话在忙——声音会一直响到下一轮结束，若没有下一轮就一直响。所以"某会话是否正在跑回合"是**向 agent 注册表询问**的（产品自己的会话列表用的也是这个来源），事件流只作兜底。另有一条 5 秒轮询，负责把"没有任何事件宣告过的变化"发布出去。
...
随包音效都是面板里**真实可选的选项**，而不是"一个真选项加一个隐藏兜底"：
...
无论槽位选了哪一个，格式都必须能被解码：底噪同时提供 Ogg 与 MP3 并排在最后兜底，所以不能播 AAC 的浏览器会落到它，而不是 404。
