...
/**
 * Shipped cues beyond the two defaults, offered as the panel's library.
 *
 * Every entry must name a file `ASSETS` already serves, so a packaging mistake
 * fails the consistency test instead of 404ing for a user. `author` and `source`
 * live here rather than only in CREDITS.md because an attribution that ships
 * inside the payload cannot get separated from the bytes it belongs to.
 */
const LIBRARY = [
  {
    id: 'let-me-go',
...
/** The shipped library entry with this id, if there is one. */
...
/** The two cue slots, and the built-in file stem each resolves to. */
const SLOTS = ['working', 'approval']
const BUILTIN_STEMS = { working: 'loop', approval: 'needs-you' }
...
function normalizeSettings(raw) {
  ...
  for (const slot of SLOTS) {
    ...
    else if (choice?.kind === 'library' && libraryFor(choice.id) !== undefined) {
      slots[slot] = { kind: 'library', id: choice.id }
  ...
  return { muted: raw?.muted === true, volume, slots }
}
...
function apply(ctx) {
  ...
  function handleAudio(req, res) {
    ...
    if (choice.kind === 'library') {
      const entry = libraryFor(choice.id)
      if (entry !== undefined) {
        sendFile(res, path.join(ASSET_DIR, entry.file), ASSETS[entry.file], cache)
        return
      }
      // The entry is gone: heal the setting rather than serving nothing.
  ...
  /**
   * `GET /library/<id>` — serve one shipped library cue, for the panel's
   * previews of entries that no slot currently uses.
   */
  function handleLibraryFile(req, res) {
    const url = new URL(req.url ?? '/', 'http://localhost')
    const id = url.pathname.slice(`${ROUTE}/library`.length).replace(/^\/+/, '')
    const entry = libraryFor(id)
    if (entry === undefined) {
      respondText(res, 404, 'dsh-audio-cue: no such library cue')
      return
    }
    sendFile(res, path.join(ASSET_DIR, entry.file), ASSETS[entry.file], IMMUTABLE)
  }
