 ...
 */
const ASSETS = {
  // The bundled tracks ship as AAC, which every browser decodes -- Safari
  // included, which is why they are not Ogg. The synthesized pair below them is
  // the decoder fallback for a browser that cannot play AAC at all.
  'let-me-go.m4a': 'audio/mp4',
  'let-me-go-ssr.m4a': 'audio/mp4',
  'loop.ogg': 'audio/ogg',
  'loop.mp3': 'audio/mpeg',
...
/**
 * Shipped cues, per slot. The first entry is the slot's default, and every entry
 * is a real choice in the panel rather than a hidden fallback: a four-second
 * synthesized pad and a three-minute track are different kinds of sound, so both
 * belong in the list.
 *
 * `stem` is the file name without its extension. The client declares which audio
 * formats it can decode, and the first cue, stem and format that all three agree
 * on wins -- so a cue offering a single format still degrades to another shipped
 * cue instead of answering 404.
 */
const BUILTIN_CUES = {
  working: [
    { id: 'let-me-go', name: 'let me go', stem: 'let-me-go' },
    { id: 'let-me-go-ssr', name: 'let me go SSR', stem: 'let-me-go-ssr' },
    // Last on purpose: it is the only cue that ships in two formats, so the
    // ordered fallback in cueOrder() ends here for anything it cannot decode.
    { id: 'loop', name: '底噪', stem: 'loop' },
  ],
  approval: [{ id: 'needs-you', name: '默认提示音', stem: 'needs-you' }],
}

/**
 * A slot's cues with the requested one first and the others behind it, so the
 * chosen cue is preferred and the rest remain as a decoder fallback. An id that
 * is not shipped falls back to the slot's default rather than serving nothing.
 * @param slot - which cue.
 * @param preferred - the chosen cue id, when there is one.
 */
function cueOrder(slot, preferred) {
  const cues = BUILTIN_CUES[slot]
  const chosen = cues.find((cue) => cue.id === preferred) ?? cues[0]
  return [chosen, ...cues.filter((cue) => cue !== chosen)]
}
...
function normalizeSettings(raw) {
  ...
  for (const slot of SLOTS) {
    ...
    if (choice?.kind === 'none') slots[slot] = { kind: 'none' }
      ...
      // A shipped cue id that no longer exists falls back to the slot's default.
      const shipped = BUILTIN_CUES[slot].some((cue) => cue.id === choice.id)
      slots[slot] = { kind: 'builtin', id: shipped ? choice.id : BUILTIN_CUES[slot][0].id }
    } else {
      slots[slot] = { kind: 'builtin', id: BUILTIN_CUES[slot][0].id }
    }
  }
...
function apply(ctx) {
  ...
  function snapshot() {
    ...
    for (const [id, state] of sessions) {
      ...
      // The registry, when there is one, decides -- exactly as the product's own
      // session list does. This is not a nicety: a turn the harness abandons
      // after an interrupt appends no `turn/end` at all, so event tracking alone
      // would hold the session open forever and the sound would never stop.
      const running = agent === undefined ? state.open : agent.status === 'running'
      if (!running) continue
      working += 1
  ...
  /** Everything a poll can compare, without the sequence number it bumps. */
  function stateKey() {
    const state = snapshot()
    return JSON.stringify({ working: state.working, waiting: state.waiting, sessions: state.sessions })
  }

  /** What the last broadcast announced, so the poll can tell whether anything moved. */
  let published = ''
  ...
  /**
   * The agent registry, when this deployment has one. It is the authority on
   * which sessions are mid-turn; this plugin's own event tracking becomes the
   * fallback for a deployment without it.
   */
  let agents
  ...
  // The registry moves without any session event when a turn is abandoned, so it
  // is polled too. Events stay the fast path; this is what keeps a change nothing
  // announced from waiting for the next heartbeat.
  const registryPoll = setInterval(() => {
    if (stateKey() !== published) broadcast()
  }, 5000)
  // The harness owns this process's lifetime, not this poll.
  registryPoll.unref?.()
  disposers.push(() => clearInterval(registryPoll))
  ...
  function statePayload() {
    return {
      ...
      // The panel builds its cue list from this, so a shipped cue that is not
      // listed here could never be chosen.
      cues: Object.fromEntries(
        SLOTS.map((slot) => [slot, BUILTIN_CUES[slot].map((cue) => ({ id: cue.id, name: cue.name }))]),
      ),
   ...
   */
  function builtinFor(slot, types, preferred) {
    for (const cue of cueOrder(slot, preferred)) {
      for (const type of types) {
        const file = `${cue.stem}.${type}`
  ...
  function handleAudio(req, res) {
    ...
    const builtin = builtinFor(slot, parseTypes(url), choice.id)
    if (builtin === null) {
