...
/** Longest file name this plugin will remember for an import. */
...
function apply(ctx) {
  ...
  function sendFile(res, abs, type) {
    ...
    res.writeHead(200, {
      ...
      // Always fresh: the files are the user's own and may be replaced in place.
      'Cache-Control': 'no-store',
    })
    res.end(bytes)
  }
  ...
  function handleAudio(res, url) {
    ...
    if (choice.kind === 'custom') {
      ...
      if (entry !== undefined) {
        sendFile(res, path.join(uploadsDir(), entry.file), entry.type)
        return
      }
    ...
    sendFile(res, builtin.abs, builtin.type)
  }
   ...
   */
  function handleUploadFile(res, url) {
  ...
  async function handleSettings(res, req) {
    if (req.method === 'GET') {
   ...
   */
  async function handleUpload(res, req, url) {
    if (req.method !== 'POST') {
   ...
   */
  function handleUploadDelete(res, req, url) {
  ...
  disposers.push(
    ctx.webServer.register({
      kind: 'prefix',
      path: `${ROUTE}/audio`,
      handler: (req, res) => handleAudio(res, new URL(req.url ?? '/', 'http://localhost')),
    }),
  )
  ...
  disposers.push(
    ctx.webServer.register({
      kind: 'prefix',
      path: `${ROUTE}/uploads`,
      handler: (req, res) => handleUploadFile(res, new URL(req.url ?? '/', 'http://localhost')),
    }),
  )
  ...
  disposers.push(
    ctx.webServer.register({
      ...
      handler: (req, res) => handleSettings(res, req),
    }),
  )
  ...
  disposers.push(
    ctx.webServer.register({
      ...
      handler: (req, res) => handleUpload(res, req, new URL(req.url ?? '/', 'http://localhost')),
    }),
  )
  ...
  disposers.push(
    ctx.webServer.register({
      kind: 'prefix',
      path: `${ROUTE}/api/uploads`,
      handler: (req, res) => handleUploadDelete(res, req, new URL(req.url ?? '/', 'http://localhost')),
    }),
  )
